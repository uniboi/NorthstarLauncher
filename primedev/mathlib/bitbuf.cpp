...
{
	bool bSucc = true;
	if (nPosition < 0 || nPosition > (int64)m_nDataBits)
	{
		SetOverflowFlag();
		bSucc = false;
		nPosition = (int64)m_nDataBits;
	}
	...
	else
	{
		...
		m_pDataIn = reinterpret_cast<uint32 const*>(reinterpret_cast<uint8 const*>(m_pData) + ((nAdjPosition / 32) << 2) + nHead);
		if (m_pData)
		{
			m_nBitsAvail = 32;
			GrabNextDWord();
...
{
	if (m_nBitsAvail >= (uint32)numbits)
	{
