...
class CBitBuffer
{
  public:
	CBitBuffer(void);
	FORCEINLINE void SetDebugName(const char* pName)
	{
		m_pDebugName = pName;
	}
	FORCEINLINE const char* GetDebugName() const
	{
		return m_pDebugName;
	}
	FORCEINLINE bool IsOverflowed() const
	{
		return m_bOverflow;
	}
	FORCEINLINE void SetOverflowFlag()
	{
		m_bOverflow = true;
	}
...
class CBitRead : public CBitBuffer
{
  public:
	...
	FORCEINLINE int64 GetNumBitsRead(void) const
	{
		return Tell();
	};
	FORCEINLINE int64 GetNumBytesRead(void) const
	{
		return ((GetNumBitsRead() + 7) >> 3);
	}

	FORCEINLINE int64 GetNumBitsLeft() const
	{
		return m_nDataBits - GetNumBitsRead();
	}
	FORCEINLINE int64 GetNumBytesLeft() const
	{
	...
	{
		return ReadUBitLong(sizeof(short) << 3);
	}
	...
	{
		return ReadUBitLong(sizeof(int32) << 3);
	}
...
class CBitWrite : public CBitBuffer
{
  public:
	...
	inline void SeekToBit(int bitPos)
	{
		m_nDataBits = bitPos;
	}
	...
	inline bool WriteBytes(const void* pIn, int nBytes)
	{
		return WriteBits(pIn, nBytes << 3);
	}

	// How many bytes are filled in?
	FORCEINLINE int GetNumBytesWritten() const
	{
		return BitByte(this->m_nDataBits);
	}
	FORCEINLINE int GetNumBitsWritten() const
	{
		return this->m_nDataBits;
	}
	FORCEINLINE int GetMaxNumBits() const
	{
		return this->m_nDataBits;
	}
	FORCEINLINE int GetNumBitsLeft() const
	{
		return this->m_nDataBits - m_nDataBits;
	}
	FORCEINLINE int GetNumBytesLeft() const
	{
		return this->GetNumBitsLeft() >> 3;
	}
	FORCEINLINE unsigned char* GetData() const
	{
		return this->m_pData;
	}
	...
	inline void SetDebugName(const char* pDebugName)
	{
...
static_assert(sizeof(bf_read) == 0x40);
...
static_assert(sizeof(bf_write) == 0x48);
...
FORCEINLINE int64 CBitRead::Tell(void) const
{
	...
	return std::min((const size_t)nCurOfs + nAdjust, m_nDataBits);
}
