...
class CModule
{
  public:
	struct ModuleSections_t
	{
		...
		inline bool IsSectionValid(void) const
		{
	...
	inline const std::vector<CModule::ModuleSections_t>& GetSections() const
	{
	...
	inline DWORD GetModuleSize(void) const
	{
		return m_nModuleSize;
	}
	inline const std::string& GetModuleName(void) const
	{
		return m_ModuleName;
	}
  ...
  private:
	CMemory FindPatternSIMD(const uint8_t* pPattern, const char* szMask, const ModuleSections_t* moduleSection = nullptr, const size_t nOccurrence = 0) const;
