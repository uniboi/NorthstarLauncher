	  ...
	  private:
		...
		static GlpiFnPtr VerifyGlpiFn_()
		{
			...
			if (pVvi)
			{
				...
				if (pVvi(&osvi, VER_MAJORVERSION | VER_MINORVERSION | VER_SERVICEPACKMAJOR | VER_SERVICEPACKMINOR, dwlMask))
				{
	...
	class Cpuid
	{
	  public:
		...
		static BOOL IsVendor(const char* strVendor)
		{
			...
			return cpu.Ebx() == *reinterpret_cast<const DWORD*>(strVendor) && cpu.Ecx() == *reinterpret_cast<const DWORD*>(strVendor + 8) && cpu.Edx() == *reinterpret_cast<const DWORD*>(strVendor + 4);
		}
	  ...
	  public:
		...
		CpuidImpl() : m_nItems(0)
		{
			...
			else
			{
				...
				for (DWORD_PTR dwThreadAffinity = 1; dwThreadAffinity && dwThreadAffinity <= dwSystemAffinity; dwThreadAffinity <<= 1)
				{
