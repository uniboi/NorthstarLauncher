...
// clang-format on
{
	...
	const char* pResult = g_pServer->m_Clients[pPlayer->m_nPlayerIndex - 1].m_ConVars->GetString(pKey, pDefaultValue);
...
// clang-format on
{
	...
	const int iResult = g_pServer->m_Clients[pPlayer->m_nPlayerIndex - 1].m_ConVars->GetInt(pKey, iDefaultValue);
...
// clang-format on
{
	...
	const float flResult = g_pServer->m_Clients[pPlayer->m_nPlayerIndex - 1].m_ConVars->GetFloat(pKey, flDefaultValue);
...
// clang-format on
{
	...
	const bool bResult = g_pServer->m_Clients[pPlayer->m_nPlayerIndex - 1].m_ConVars->GetInt(pKey, bDefaultValue);
